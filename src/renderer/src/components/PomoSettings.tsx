...
export function PomoSettings({ preferences, onSave }: PomoSettingsProps): JSX.Element {
  ...
  return (
    <div className="flex flex-col items-center justify-center space-y-5">
      ...
      <div className="flex flex-col items-center justify-center">
        <label className="text-gray-700 text-sm font-bold mb-2" htmlFor="pomodoroCount">
          Pomodoro Count
        </label>
        <input
          className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
          ...
          onChange={(e): void => {
            const value = parseInt(e.target.value);
            validateInput(value, 'pomodoroCount') &&
              setSettings({ ...settings, pomodoroCount: value });
          }}
        />
        <span className="text-red-500">{errorMessages.pomodoroCount}</span>
      </div>
      <button
